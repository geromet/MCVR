...
std::vector<std::string> explicitEnvironment(const char *fixture, const std::string &attempt, const std::string &caseId) {
    ...
    for (char **p = environ; p && *p; ++p) {
        std::string entry(*p);
        if (startsWith(entry, std::string(KEY).append("=").c_str()) ||
            startsWith(entry, "MCVR_TEST_ATTEMPT=") || startsWith(entry, "MCVR_TEST_CASE=")) continue;
...
TEST(thread_env_child_explicit_fixture_ignores_conflicting_parent) {
    ...
    setenv(KEY, "77", 1); // mechanically after owned map construction
